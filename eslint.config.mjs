...
import descriptive from "@ubccpsc310/eslint-plugin-descriptive";
...
// update the rulses for the /src/ directory here
const srcRules = {
	// safety rules
...
// update the rules for the /test/ directory here!
const testRules = {
	...
	"@ubccpsc310/descriptive/max-nested-callbacks": ["error", 5],
	"max-nested-callbacks": ["error", 5],
	"@ubccpsc310/descriptive/no-magic-numbers": "off",
};
...
export default [
	{
		...
		ignores: [
			...
			"frontend/**/*", // ignore frontend files
		],
		languageOptions: {
			parser: tseslint,
			parserOptions: {
		...
		plugins: {
			"@ubccpsc310/descriptive": descriptive,
			prettier: prettier,
		},
		rules: srcRules,
	},
